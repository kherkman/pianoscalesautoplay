(function() {
    ...
    function applyScaleFilter() { 
        ...
        if (window.SongMakerSystem) window.SongMakerSystem.stop(true); 
    ...
    function toggle88Keys() { 
        ...
        // Notify SheetMusic to increase canvas height and staff spacing
        if (window.SheetMusic && typeof window.SheetMusic.set88KeyMode === 'function') {
            window.SheetMusic.set88KeyMode(is88KeyMode);
        }
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (window.AutoPlaySystem) {
            window.AutoPlaySystem.init({
                ...
                stopSongMaker: () => { if (window.SongMakerSystem) window.SongMakerSystem.stop() },
                getIsSongMakerPlaying: () => window.SongMakerSystem ? window.SongMakerSystem.isPlaying() : false
            });
        }

        // --- Pass Dependencies to the Extracted SongMaker System ---
        if (window.SongMakerSystem) {
            window.SongMakerSystem.init({
                initAudioContext: initAudioContext,
                playNote: playNote,
                stopAudioForNote: stopAudioForNote,
                getFullScaleRange: getFullScaleRange,
                getKeysData: () => keysData,
                getEffectiveBaseTempoMs: () => effectiveBaseTempoMs,
                syncTempo: syncTempo,
